...
const BottomNav: React.FC = () => {
  ...
  return (
    <nav className="flex items-center bg-black/60 backdrop-blur-3xl border border-white/20 rounded-[2.5rem] p-2.5 shadow-[0_25px_60px_rgba(0,0,0,0.8)] border-b-primary/20">
      {navItems.map((item) => (
        <NavLink
          key={item.to}
          to={item.to}
